...
from baselines.ppo2.policies import MlpPolicy_Goal as MlpPolicy
...
def run_test():
    ...
    env.set_sim_data(sim_data, obs['achieved_goal'])
    # print('get data', sim_data)
    # print('goal', obs['achieved_goal'])
    ...
    for j in range(100):
        ...
        for i in range(100):
            ...
            # obs_pre = obs.copy()
            # actions[0][-1] *= 0.1
            full_obs, r, done, _ = env.step(actions[0])
            obs = np.concatenate((full_obs['observation'], full_obs['desired_goal']), axis=1) 
