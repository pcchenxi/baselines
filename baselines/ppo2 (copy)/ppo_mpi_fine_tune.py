...
def update_pf_sets(pf_sets, candidate_set, sub_target):        
    ...
    if pf_sets == []:
        ...
        for i in range(len(pf_sets)):
            if np.all(np.asarray(candidate_set[0:3]) < np.asarray(pf_sets)[i][0:3]): # pf_sets[i] is pareto dominated by the candidate, remove pf_sets[i] and add candidate to the pf_set
                add_candidate = False
                break
...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    # checkdir = osp.join('../exp_models/exp_model_landing', 'meta_main')
    ...
    #         random_ws.append(random_w)
    #     np.save('../model/checkpoints/ws'+'.npy', random_ws)
    ...
    for update in range(start_update+1, nupdates+1):
        ...
        if comm_rank == 30:
            ...
            random_w = random_ws[comm_rank][0:REWARD_NUM]
        # if comm_rank == 0:
        #     random_w = [1, 1,  1,  1]
        # elif comm_rank == 1:
        #     random_w =[1, 0,  0,  0]
        ...
        for g in range(num_g):
            ...
            save_results = mean_returns*1
            save_results = np.append(save_results, np.sum(np.multiply(mean_returns, random_w)))
            # print('s', save_results)
            fine_tunning_results.append(save_results)
            np.save('../model/checkpoints/results_'+str(comm_rank)+'.npy', fine_tunning_results)

            if comm_rank == 0 and g > 0:
                area = 0
                # for mean_return in mean_return_gather:
                #     is_pf, is_convax, pf_sets, pf_sets_convex, area = update_pf_sets(pf_sets, mean_return, [])
                # model.write_summary('pf_performance/hv_indicator', area, update)
                print(g, len(fine_tunning_results), comm_rank, '    ', np.array_str(mean_returns, precision=3, suppress_small=True), np.sum(np.multiply(mean_returns, random_w)))
                # if g%5 == 0:
