...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    for update in range(start_update+1, nupdates+1):
        ...
        for i in range(20): 
            ...
            random_w = np.random.rand(REWARD_NUM) - 0.5
            ...
            v_loss_pre = 0
            for k in range(50):
                obs, returns, masks, actions, values, rewards, neglogpacs, states, epinfos, ret = runner.run(int(64), is_test=False) #pylint: disable=E0632
            ...
            for g in range(num_g):
                ...
                # keep_weight = update_pf_sets(pf_sets, np.mean(returns_p, axis = 0), v_loss)
                print('    ', np.array_str(mean_returns, precision=3, suppress_small=True), np.sum(np.multiply(mean_returns, random_w)))
                ...
                mblossvals = nimibatch_update(  nbatch, noptepochs, nbatch_train,
                                                ...
                                                lr_s, cr_s, states, nsteps, model, update_type = 'actor') 
                v_loss = nimibatch_update(  nbatch, noptepochs, nbatch_train/10,
                                                obs_p, returns_p, masks_p, actions_p, values_p, advs_p, neglogpacs_p,
                                                lr_s, cr_s, states, nsteps, model, update_type = 'value') 
                print(v_loss[0])
                display_updated_result( mblossvals, update, log_interval, nsteps, nbatch, 
            ...
            mean_returns_p = np.mean(returns_p, axis = 0)
            w_v = np.sum(np.multiply(mean_returns_p, target_w))
            print(' eva', np.array_str(mean_returns_p, precision=3, suppress_small=True), w_v)
            w_vs.append(w_v)
            w_returns.append(mean_returns_p)
            ...
            for r_index in range(REWARD_NUM):
                model.write_summary('meta/mean_ret'+str(r_index+1), np.mean(returns_p[:,r_index]), update)
