...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    for update in range(start_update+1, nupdates+1):
        ...
        advs_task = advs_ori[:, -2] #+ advs_ori[:, 1]*2
        advs_c = advs_ori[:, -1] #+ advs_ori[:, 1]*2

        advs_task = (advs_task - advs_task.mean()) / (advs_task.std() + 1e-8)
        advs_c = (advs_c - advs_c.mean()) / (advs_c.std() + 1e-8)

        scale = 0
        advs = advs_task*scale + advs_c*(1-scale)
