...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, goal_space, nbatch_act, nbatch_train,
        ...
        def train(lr, cliprange, obs, obs_next, returns, actions, values, advs, rewards, neglogpacs, train_type='all'):
            ...
            if train_type == 'all':
                ...
                return sess.run(
                    [pg_loss, loss_v, loss_inverse, loss_forward, entropy, approxkl, clipfrac, _grad_norm_a, train_a, train_v],
                    td_map                
                )[:-2]
...
class Runner(object):
    ...
    def init_task_pool(self, nsteps):
        self.tasks = []
        ...
        for t in range(nsteps):
            ...
            task_value = self.model.value([task_obs])
            print(task_value)
    ...
    def run(self, nsteps, is_test = False, use_deterministic = False):
        ...
        for t in range(nsteps):
            if use_deterministic:
                actions, values, neglogpacs = self.model.step_max(self.obs, self.dones)
            else:
                actions, values, neglogpacs = self.model.step(self.obs, self.dones)
        ...
        last_values = self.model.value(self.obs, self.dones)
