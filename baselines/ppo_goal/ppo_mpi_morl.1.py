...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    params_trace_rand = model.get_current_params(params_type='eir')

    # checkdir = osp.join('../model', 'checkpoints')    
    # mocel_path = osp.join(checkdir, str('6'))    
    ...
    for update in range(start_update+1, nupdates+1):
        ...
        obs, obs_next, returns, dones, actions, values, advs_ori, rewards, fixed_state_f, pm_states, neglogpacs, epinfos = runner.run(model, int(nsteps), pm_cut, pm_scale, boost=boost)
        ...
        dones_gather = MPI.COMM_WORLD.gather(dones)
        ...
        if comm_rank == 0:
            ...
            all_dones = np.asarray(dones_gather).reshape(nprocs*length)
            ...
            if mode == 'av':
                ...
                if save_index != int(save_count/save_interval) or add:
                    print('save !!!!!!!!!!!!!', len(pm_buffer_obs))
                    if pm_buffer_obs == []:
                        pm_buffer_obs, pm_buffer_obs_next, pm_buffer_a, pm_buffer_fst, pm_buffer_rew, pm_buffer_dones = \
                                all_obs.copy(), all_obs_next.copy(), all_a.copy(), all_fsf.copy(), all_reward.copy(), all_dones.copy()
                    else:
                        ...
                        pm_buffer_dones = np.concatenate((pm_buffer_dones, all_dones.copy()), axis=0)
                ...
                if len(pm_buffer_obs) >= nprocs*length*save_index:
                    # print('reset trace net')
                    # model.replace_params(params_trace_rand, params_type='eir') 
